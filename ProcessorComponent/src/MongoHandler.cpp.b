...
public:
  ...
  template <typename T>
  OperationResult findModels(const std::string &collectionName,
...
template <typename T>
OperationResult MongoDBHandler::storeModel(const std::string &collectionName,
                                           const T &model) {
  try {
    ...
    std::cout << "Successfully stored model in collection: " << collectionName
              << std::endl;
...
template <typename T>
OperationResult MongoDBHandler::findModels(const std::string &collectionName,
                                           const std::string &queryJson) {
  try {
    auto collection = db[collectionName];
    ...
    std::cout << "Successfully retrieved " << models.size()
              << " models from collection: " << collectionName << std::endl;
